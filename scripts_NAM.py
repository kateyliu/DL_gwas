...
def readData(input):
	data = pd.read_csv(input,sep='\t',header=None,na_values='nan')
	SNP = data.iloc[:,2:].convert_objects(convert_numeric=True).values
	YLD = data.iloc[:,1].convert_objects(convert_numeric=True).values
...
def multi_CNN(input):
	...
	x1 = Conv1D(10,4,padding='same',activation = 'linear', kernel_initializer = 'TruncatedNormal',kernel_regularizer=regularizers.l2(0.1),bias_regularizer = regularizers.l2(0.01))(inputs)
	
	x2 = Conv1D(10,20,padding='same',activation = 'linear', kernel_initializer = 'TruncatedNormal',kernel_regularizer=regularizers.l2(0.1),bias_regularizer = regularizers.l2(0.01))(inputs)
	
	x = concatenate([x1,x2]) 
	
	x = Dropout(0.75)(x)
	
	x = Conv1D(10,20,padding='same',activation = 'linear', kernel_initializer = 'TruncatedNormal',kernel_regularizer=regularizers.l2(0.01),bias_regularizer = regularizers.l2(0.01))(x) 
	
	x = Flatten()(x)
	
	x = Dropout(0.75)(x)
	
	outputs = Dense(1,activation = isru,kernel_initializer = 'glorot_normal',bias_regularizer = regularizers.l2(0.01))(x)
...
# a= 0.5
def isru(x):
	return  x/(K.sqrt(1+0.5*K.square(x)))
	
	
def main():
	
	polyp = pd.read_csv("polytest.txt",sep='\t',header=0,na_values='nan')
	wald =  polyp.iloc[:,0].convert_objects(convert_numeric=True).values
	...
	for i in range(1,11):
		...
		trainSNP, trainYLD  = readData("SoyNAM/imp/4236_imp_train_"+str(i)+".txt")
		testSNP, testYLD = readData("SoyNAM/imp/4236_imp_test_"+str(i)+".txt")	
		valSNP, valYLD = readData("SoyNAM/imp/4236_imp_val_"+str(i)+".txt")	
		
		model = resnet(trainSNP)
		
		history  = model.fit(trainSNP, trainYLD, batch_size=batch_size, epochs=1000,validation_data=(valSNP,valYLD),callbacks=[early_stopping],shuffle= True)
		preytest = model.predict(testSNP)
		preytest.shape = (preytest.shape[0],)
		
		corr = pearsonr(preytest,testYLD)[0]
		test_corr.append(float('%0.4f' % corr))
		
		print ('Accuracy of folds'+ str(i) +'is :' + str(corr))
		
		tmp = get_saliency(testSNP,model)
		...
		if i == 1:
			...
			saliency = np.concatenate((saliency,tmp),axis=-1)
		
		
	print ('Accuracy of 10 fold cross validation is '+ str(np.mean(test_corr)))
	show_images_plot(saliency,wald,"saliency_out.png")
